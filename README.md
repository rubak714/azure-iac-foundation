...
This project runs at near-zero cost.

Resource groups, Bicep deployments, RBAC assignments, and Azure Policy are all free. Log Analytics data ingestion and Key Vault operations stay within the free monthly allowance for a project this small. A full session costs around **0.05 to 0.10 euros**. The entire project across all four modules should cost under **1 euro** total.

I delete or deallocate anything expensive after each session. I always mention it in the README when I do that.

---

## ☁️ Deploy it yourself

Once Module 2 is merged, the whole foundation deploys with one command:
...
Module 1 uses the Azure CLI directly to create the first resource group by hand. That is a bootstrap step and it is deliberate. The walkthrough below explains why.
